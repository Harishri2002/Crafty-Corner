...
if(true){
    ...
    $pcid=$_POST['pcid'];
    $crid=$_GET['crid'];
    $stmt=$admin->ret("SELECT * FROM `cart` INNER JOIN `product` ON product.p_id=cart.p_id WHERE `c_id`='$cus_id' AND `cr_id`='$crid'");
    $un=uniqid();
   ...
   while( $row=$stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    $stmt6=$admin->cud("DELETE FROM `cart` WHERE `c_id`='$cus_id' AND `p_id`=$pid",'DELETED');
